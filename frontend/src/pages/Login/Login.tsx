...
const Login: React.FC = () => {
  ...
  const [email, setEmail] = React.useState(""); // empty by default
  const [password, setPassword] = React.useState(""); // placeholder; not validated
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Persist session info (for other pages like APO_PreEnlistment)
      localStorage.setItem("animo.user", JSON.stringify(user));

      // Role-based redirect (dynamic from DB)
      if (user.roles.includes("apo")) {
          ...
          <form className="space-y-4 sm:space-y-5" onSubmit={onSubmit}>
            ...
            >
              ...
              >
                <path
                  fill="#FFC107"
                  d="M43.611 20.083H42v-.083H24v8h11.303C33.668 32.657 29.223 36 24 36c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.153 7.957 3.043l5.657-5.657C32.671 6.053 28.568 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20c10.5 0 19.5-7.6 20-19.917v-4z"
                />
                <path
                  fill="#FF3D00"
                  d="M6.306 14.691l6.571 4.818C14.655 16.157 18.961 13 24 13c3.059 0 5.842 1.153 7.957 3.043l5.657-5.657C32.671 6.053 28.568 4 24 4 16.102 4 9.287 8.343 6.306 14.691z"
                />
                <path
                  fill="#4CAF50"
                  d="M24 44c5.313 0 10.18-1.822 13.989-4.943l-6.476-5.318C29.71 35.41 27.001 36 24 36c-5.275 0-9.747-3.555-11.346-8.355l-6.51 5.02C9.147 39.793 16.02 44 24 44z"
                />
                <path
                  fill="#1976D2"
                  d="M44 24c0-1.341-.138-2.652-.389-3.917H24v8h11.303c-.737 3.484-2.812 6.086-5.39 7.66l6.476 5.318C39.485 37.65 44 31.45 44 24z"
                />
              </svg>
