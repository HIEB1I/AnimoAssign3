...
import { Search, Bell, UserCircle, Inbox, LogOut, Plus, ChevronLeft } from "lucide-react";
...
/* ====== Top Bar (same style, Chair labels) ====== */
function TopBar() {
  ...
  useEffect(() => {
    ...
    const setHeightVar = () => document.documentElement.style.setProperty("--header-h", `${el.offsetHeight}px`);
    setHeightVar();
        ...
        <div className="mx-auto flex w-full items-center justify-between px-5 py-4">
          <div ref={wrapperRef} className="relative">
            <button onClick={() => setMenuOpen(o => !o)} className="group flex items-center gap-3 rounded-lg px-2 py-1 hover:bg-white/10">
              <span className="grid h-10 w-10 place-items-center rounded-full bg-white/20"><UserCircle className="h-6 w-6" /></span>
              <span className="leading-tight text-left">
                <div className="text-[17px] font-semibold">Rafael Cabredo</div>
                <div className="text-[12px] opacity-90">Faculty</div>
              </span>
            </button>
            {menuOpen && (
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => navigate("/faculty/inbox")} className="rounded-md p-2 hover:bg-white/15" title="Inbox">
...
export default function CHAIR_Inbox() {
  ...
  const goBack = () => navigate("/faculty/overview");
