...
export default function CHAIR_FacultyService() {
  // Receiver department picker (affects only Received table)
  const [activeReceiverDept, setActiveReceiverDept] = useState<string>(RECEIVER_DEPTS[0]);
  ...
  // Fetch rows for requester + active receiver, then merge
  async function refresh() {
    setLoadingList(true);
    try {
      const [reqSide, recSide] = await Promise.all([
        listFacultyService({ dept: REQUESTER_DEPT }),
        listFacultyService({ dept: activeReceiverDept }),
      ]);
      const a = (reqSide?.rows || []) as FacultyServiceRow[];
      const b = (recSide?.rows || []) as FacultyServiceRow[];
      const map = new Map<string, FacultyServiceRow>();
      for (const r of [...a, ...b]) if (r.fs_id) map.set(r.fs_id, r);
  ...
  const sentRows = rows.filter((r) => eqDept(r.from_department, REQUESTER_DEPT));
  const receivedRows = rows.filter((r) => eqDept(r.to_department, activeReceiverDept));
  ...
  // Preload faculty and reset edits on receiver change
  useEffect(() => {
    ensureFacultyForDept(activeReceiverDept);
    setEdits({});
    refresh().catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeReceiverDept]);
    ...
    <div className="min-h-screen w-full bg-gray-50 text-slate-900 px-8 py-8">
      <header className="mb-4">
        ...
        <p className="text-sm text-neutral-600">
          Create requests, track sent requests, and respond as the receiving department — all in one
          screen.
        </p>
      </header>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-x-auto overflow-y-visible mb-8">
        <div className="px-5 pt-4 text-[13px] text-neutral-600">Create Request</div>
      ...
      {/* Receiver filter just above the Received Requests table */}
      <div className="mt-8 mb-3 flex items-center gap-3">
        <span className="text-[13px] text-neutral-600">Act as department (receiver):</span>
        <Dropdown
        value={activeReceiverDept}
        onChange={(v) => {
          setActiveReceiverDept(v);
          ensureFacultyForDept(v);
          setEdits({});
          refresh();
        }}
        options={[...RECEIVER_DEPTS]}
        placeholder="Select department…"
      ...
      {/* 3) RECEIVED REQUESTS (editable, full; based on activeReceiverDept) */}
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-x-auto overflow-y-visible">
