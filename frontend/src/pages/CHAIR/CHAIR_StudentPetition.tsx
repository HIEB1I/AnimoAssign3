// frontend/src/pages/CHAIR/CHAIR_StudentPetition.tsx
...
export default function CHAIR_StudentPetition() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setErr("");
        const { ok, rows } = await listChairSP({ status, search });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const { rows } = await listChairSP({ status, search });
      setRows(rows);
  ...
  const forwardSelected = async () => {
    ...
    try {
      ...
      await bulkForwardChairSP(selected, target);
      const { rows } = await listChairSP({ status, search });
      setRows(rows);
      setSelected([]);
    } catch (e: any) {
