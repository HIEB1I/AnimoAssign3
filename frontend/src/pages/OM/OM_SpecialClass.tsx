...
export default function OM_SpecialClass({ hideMessageIcon = false }: { hideMessageIcon?: boolean } = {}) {
  ...
  const roomCellItems = (r: Partial<OMSpecialClassRow>) => {
    const slots = [1, 2].flatMap((slot) => {
      const room = roomLabel(r, slot as 1 | 2);
      const title = roomTitle(r, slot as 1 | 2);
      const hasAnyValue = [
        slot === 1 ? r.day1 : r.day2,
        slot === 1 ? r.begin1 : r.begin2,
        slot === 1 ? r.end1 : r.end2,
        slot === 1 ? r.room1 : r.room2,
        slot === 1 ? r.room_id1 : r.room_id2,
      ].some((value) => String(value || "").trim());

      return hasAnyValue
        ? [{ key: `room-${slot}`, label: room || "—", title }]
        : [];
    });

    return slots.length ? slots : [{ key: "room-empty", label: "—", title: undefined }];
  };
